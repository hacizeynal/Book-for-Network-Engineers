...
trunk_config = {
    "FastEthernet0/1": [ 10, 20, 30, 40, 50, 60 ],
    "FastEthernet0/2": [ 11, 30 ],
    "FastEthernet0/4": [ 17 ],
    "FastEthernet0/5": [ 18 ],
}
...
def generate_trunk_config(intf_vlan_mapping, trunk_template):
    for interface, vlan in intf_vlan_mapping.items():
        trunk_configuration.append("interface {}".format(interface))
        for config in trunk_template:
            if config.endswith("allowed vlan"):
                vlans_to_string = ",".join(str(vlans) for vlans in vlan)
                trunk_configuration.append(f'{config} {vlans_to_string}')
            else:
                trunk_configuration.append(f"{config}")
    print(trunk_configuration)
